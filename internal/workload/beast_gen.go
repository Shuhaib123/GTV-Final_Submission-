//go:build workload_beast
// +build workload_beast
...
func RunbeastProgram(__jspt_ctx_5 context.Context) {
	var __jspt_wg_0 sync.WaitGroup
	defer __jspt_wg_0.Wait()
	__jspt_ctx_5, __jspt_task_6 := trace.NewTask(__jspt_ctx_5, "beast")
	defer __jspt_task_6.End()
	trace.Log(__jspt_ctx_5, "main", "beast starting")
	s := new(Server)
