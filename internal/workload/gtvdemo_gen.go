//go:build workload_gtvdemo
// +build workload_gtvdemo
...
func RungtvdemoProgram(__jspt_ctx_8 context.Context) {
	...
	__jspt_ctx_8, __jspt_task_9 := trace.NewTask(__jspt_ctx_8, "gtvdemo")
	defer __jspt_task_9.End()
	trace.Log(__jspt_ctx_8, "main", "gtvdemo starting")
...
func __jspt_recv_value_6[T any](__jspt_ctx_22 context.Context, label string, ch <-chan T) T {
	if trace.
		IsEnabled() {
		trace.Log(__jspt_ctx_22, "ch_ptr",

			fmt.Sprintf("ptr=%p",

				ch))
		trace.Log(__jspt_ctx_22, "ch_name", fmt.Sprintf("ptr=%p name=%s", ch, label,
		))
	}
	var v T
...
func __jspt_recv_value_ok_7[T any](__jspt_ctx_22 context.Context, label string, ch <-chan T) (T,

	bool) {
	if trace.IsEnabled() {
		trace.Log(__jspt_ctx_22, "ch_ptr", fmt.Sprintf("ptr=%p",

			ch))
		trace.Log(__jspt_ctx_22, "ch_name", fmt.Sprintf("ptr=%p name=%s",
	...
	var v T
	var ok bool
	trace.WithRegion(__jspt_ctx_22, label,
		func() {
			v,
				ok = <-ch
		})
