//go:build workload_idealo12
// +build workload_idealo12
...
func worker(__jspt_ctx_11 context.Context, workerID int, jobs <-chan Job, results chan<- Result, wg *sync.WaitGroup) {
	defer func() {
		trace.WithRegion(__jspt_ctx_11, "wg.done wg", func() {
			{
				wg.Done()
			}
		})
		...
		if trace.IsEnabled() {
			trace.
				Log(__jspt_ctx_11,
					...
					fmt.Sprint(Result{JobID: job.ID, Value: val +
						workerID}),
				)
		}
		if trace.IsEnabled() {
...
func Runidealo12Program(__jspt_ctx_8 context.Context) {
	...
	__jspt_ctx_8, __jspt_task_9 := trace.NewTask(__jspt_ctx_8, "idealo12")
	defer __jspt_task_9.End()
	trace.Log(__jspt_ctx_8, "main", "idealo12 starting")
...
func __jspt_recv_value_ok_7[T any](__jspt_ctx_22 context.
	Context, label string, ch <-chan T) (T, bool) {
	if trace.IsEnabled() {
		trace.
			Log(__jspt_ctx_22, "ch_ptr", fmt.Sprintf("ptr=%p", ch))
		trace.
			Log(__jspt_ctx_22,
				"ch_name",
...
func init() {
	RegisterWorkload("idealo12", Runidealo12Program)
}
