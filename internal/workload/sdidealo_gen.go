//go:build workload_sdidealo
// +build workload_sdidealo
...
func RunsdidealoProgram(__jspt_ctx_8 context.Context) {
	...
	__jspt_ctx_8, __jspt_task_9 := trace.NewTask(__jspt_ctx_8, "sdidealo")
	defer __jspt_task_9.End()
	trace.Log(__jspt_ctx_8, "main", "sdidealo starting")
		...
		trace.WithRegion(__jspt_ctx_28, "goroutine: anon", func() {
			{
				...
				defer func() {
					trace.WithRegion(__jspt_ctx_8, "wg.done wg", func() {
						{
							wg.Done()
						}
					})
			...
			ctx, "worker: receive from "+"c", timeout.C):
				...
				if trace.IsEnabled() {
					trace.
						Log(ctx,
							"select",
							fmt.Sprintf("select_case id=%s idx=2 kind=recv ptr=%p name=%q",

								__jspt_sel_22,

								timeout.C, "worker: receive from "+
									"c"))
				}
				if trace.IsEnabled() {
					trace.
...
func init() {
	RegisterWorkload("sdidealo", RunsdidealoProgram,
	)
}
