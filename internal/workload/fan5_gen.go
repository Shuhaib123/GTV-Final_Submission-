...
func Runfan5Program(__jspt_ctx_8 context.Context) {
	...
	__jspt_ctx_8, __jspt_task_9 := trace.NewTask(__jspt_ctx_8, "fan5")
	defer __jspt_task_9.End()
	trace.Log(__jspt_ctx_8, "main", "fan5 starting")
	...
	// Stage 1: generator
	func(__jspt_ctx_18 context.Context,) {
	...
	go func(__jspt_ctx_20 context.Context,) {
		trace.WithRegion(__jspt_ctx_20, "goroutine: anon",
				...
				for {
					{
						...
						case <-__jspt_recv_in_region_4(__jspt_ctx_8, "worker: receive from "+"chan", ctx.Done()):
							...
							return
						case it, ok := <-__jspt_recv_in_region_4(__jspt_ctx_8, "worker: receive from "+"out", out):
							if trace.IsEnabled() {
								trace.Log(__jspt_ctx_8,
...
func __jspt_select_recv_3[
	T any](__jspt_ctx_22 context.Context, label string, ch <-chan T) <-chan T {
	if trace.IsEnabled() {
...
func __jspt_recv_in_region_4[T any](__jspt_ctx_22 context.
...
func __jspt_select_send_5[T any](__jspt_ctx_22 context.Context, label string, ch chan<- T) chan<- T {
	if trace.IsEnabled() {
		...
		trace.Log(__jspt_ctx_22, "ch_name",
			fmt.Sprintf("ptr=%p name=%s",
				ch,
				label,
			))
